...
! draw grid
  ...
  call fglNewList(3,GL_COMPILE)
  call fglDepthRange(0.00001d0,0.99999d0)
  wi=CHGLyGridLineW
  call fglLineWidth(wi)
  jk=1
  do j=1,nObjWeb
...
subroutine ChGLsaveBMP(name)
  Implicit none
  Integer(4) il,iw,ih,ii
  Integer(4), allocatable :: buff(:)
  integer(4), allocatable :: ibuff(:)
  integer(1), allocatable :: i1buff(:),i1AVIbuff(:)
  real(4), dimension(4) :: cRGBA
  Real(4) d,dmin,r,g,b
  ...
  call fglutSetCursor(GLUT_CURSOR_WAIT)
  iw=iCHGLWidthCurrent
  ih=iCHGLHeightCurrent
  if(iCHGLmode.gt.0_2) then
    allocate(ibuff(3*iw),i1buff(iw))
  else
    allocate(buff(iw*ih),i1buff(iw)) ! I don't know why this works but not buff(3*iw) - scan row by row - or buff(3) - scan pixels
  end if
  c2='BM'
  iBmpHeaderSize=40
  iBmpDataOffset=1078
  ...
  do il=0_4,ih-1_4
    if(iCHGLmode.gt.0_2) then
    !  call fglReadPixels(0_4,il,10_4,1_4,GL_COLOR_INDEX,GL_INT,iibuff)
    !  do i1=1,iw
    ...
    !  end do
      ...
      do ii=0_4,iw-1_4
      i1=ii+1
   !   call fglReadPixels(ii,il,2_4,2_4,GL_RGB,GL_SHORT,buff)
  buff(1:3)=100
        i1buff(i1)=0_1
        imin=0_2
        dmin=1.0
  ...
  call fglutSetCursor(GLUT_CURSOR_RIGHT_ARROW)
...
! called when a mouse button changes
  Implicit none
  integer(2),intent(in out) :: button,state,x,y
...
! Determine and apply the button function
  ...
  case(ZOOM)
    if(y<begin%y) then
      factor=1.08/(1.08+.0028*(begin%y-y))
    else if(y>begin%y) then
      factor=1.08+.0028*(y-begin%y)
    else
      factor=1.08
    end if
  ...
  case(SCALEX)
    if(y<begin%y) then
      factor=1.08+.0028*(begin%y-y)
    else if(y>begin%y) then
      factor=1.08/(1.08+.0028*(y-begin%y))
    else
      factor=1.08
    end if
...
! update private variables and redisplay
  ...
  if(moving_left.or.moving_middle) call fglutPostRedisplay
...
! handles the arrow key operations
  USE CHDLG
  Implicit none
  integer(2),intent(in out) :: key,x,y
  real(8) :: factor
  character(12) name
  ...
  if(key.eq.GLUT_KEY_F10) then
    ...
    case(ROTATE)
      select case(key)
      case(GLUT_KEY_LEFT)
        angle%x=angle%x-1.08
      case(GLUT_KEY_RIGHT)
        angle%x=angle%x+1.08
      case(GLUT_KEY_DOWN)
        angle%y=angle%y+1.08
      case(GLUT_KEY_UP)
        angle%y=angle%y-1.08
      end select
    case(SCALEX)
...
! converts a 3D point from cartesean to spherical coordinates
  ...
  if(x==0.08.and.y==0.08) then
    spoint%phi=-0.5d0*PI !0.08
  else
...
! draw vectors
  ...
  if(d.gt.1.0d-100) then
    call fglBegin(GL_LINES)
    call fglLineWidth(1.0)
    if(iC.lt.0) call fglLineWidth(2.0)
...
! generate OpenGL movie
  Implicit none
    Include 'resource.fd'
    Integer(4) i,j,kk,k0,k1,k2,k3,k4,lTxt,idum,lout
    Logical ldum
    Character(32) Txt
    call fglutSetCursor(GLUT_CURSOR_WAIT)
   ...
   !! if(lAVIopen) idum=closeAVIfile()
    ...
    do while(lRestart)
      lRestart=.false.
! find the end mark positions k1,k2,k3
      k0=nMovCommand+1
      k1=nMovCommand+1
      k2=nMovCommand+1
      k3=nMovCommand+1
      ...
      do kMovCommand=1,nMovCommand
        ...
        if(Mov_Command(1:3).eq.'end') then
          if(kk.eq.0) k0=kMovCommand
          if(kk.eq.1) k1=kMovCommand
          if(kk.eq.2) k2=kMovCommand
          if(kk.eq.3) k3=kMovCommand
          if(kk.eq.4) then
            k4=kMovCommand
            exit
          end if
...
! generate the movie
      idum=0
      do kMovCommand=1,k0-1 ! pre-processing loop
        call GetStrInStr(Mov_Commands,lsMovCommands-1,kMovCommand,Mov_Command,lsMovComm)
        if(lsMovComm.lt.1) exit
        ...
        call IntToStr(k0-1,0,0,SpaceText,lout)
        call OutTxt('m3',SpaceText(1:lout))
        !!! call SleepQQ(1_4)
        call MovieCommand(.true.,idum)
      ...
      if((.not.lRestart).and.(.not.lStopThread).and.(idum.le.0)) then
        do j=1,nMovSeq ! outer loop
          do kMovCommand=k0+1,k1-1
            call GetStrInStr(Mov_Commands,lsMovCommands-1,kMovCommand,Mov_Command,lsMovComm)
            if(lsMovComm.lt.1) exit
            call OutTxt('t3','B:'//Mov_Command(1:lsMovComm)//Char(0))
            call IntToStr(Int4(j),0,0,SpaceText,lout)
            call OutTxt('n3',SpaceText(1:lout))
            call IntToStr(nMovSeq,0,0,SpaceText,lout)
            call OutTxt('m3',SpaceText(1:lout))
            !!! call SleepQQ(1_4)
            call MovieCommand(.true.,idum)
            if(lDrawOGL) call CHGLDraw()
          ...
          if(lStopThread.or.(idum.gt.0).or.(lsMovComm.lt.1)) Exit
          do i=1,nMovPic ! inner loop
            do kMovCommand=k1+1,k2-1
              call GetStrInStr(Mov_Commands,lsMovCommands-1,kMovCommand,Mov_Command,lsMovComm)
              if(lsMovComm.lt.1) exit
              call OutTxt('t3','C:'//Mov_Command(1:lsMovComm)//Char(0))
              call IntToStr(Int4(j),0,0,SpaceText,lout)
              call IntToStr(Int4(i),0,0,Txt,lTxt)
              call OutTxt('n3',SpaceText(1:lout)//'/'//Txt(1:lTxt)//Char(0))
              call IntToStr(nMovSeq,0,0,SpaceText,lout)
              call IntToStr(Int4(nMovPic),0,0,Txt,lTxt)
              call OutTxt('m3',SpaceText(1:lout)//'/'//Txt(1:lTxt)//Char(0))
              !!! call SleepQQ(1_4)
              call MovieCommand(.true.,idum)
              if(lDrawOGL) call CHGLDraw()
              if(lSaveOGLavi) call ChGLsaveBMP()
          ...
          if(lStopThread.or.(idum.gt.0).or.(lsMovComm.lt.1)) Exit
          do kMovCommand=k2+1,k3-1
            call GetStrInStr(Mov_Commands,lsMovCommands-1,kMovCommand,Mov_Command,lsMovComm)
            if(lsMovComm.lt.1) exit
            call OutTxt('t3','D:'//Mov_Command(1:lsMovComm)//Char(0))
            call IntToStr(Int4(j),0,0,SpaceText,lout)
            call OutTxt('n3',SpaceText(1:lout))
            call IntToStr(nMovSeq,0,0,SpaceText,lout)
            call OutTxt('m3',SpaceText(1:lout))
      ...
      if((.not.lRestart).and.(.not.lStopThread).and.(idum.le.0)) then
        do kMovCommand=k3+1,k4-1 ! post-processing loop
          call GetStrInStr(Mov_Commands,lsMovCommands-1,kMovCommand,Mov_Command,lsMovComm)
          if(lsMovComm.lt.1) exit
          call OutTxt('t3','E:'//Mov_Command(1:lsMovComm)//Char(0))
          call IntToStr(Int4(kMovCommand),0,0,SpaceText,lout)
          call OutTxt('n3',SpaceText(1:lout))
          call IntToStr(k3-1,0,0,SpaceText,lout)
          call OutTxt('m3',SpaceText(1:lout))
        !!! call SleepQQ(1_4)
          call MovieCommand(.true.,idum)
          if(lDrawOGL) call CHGLDraw()
